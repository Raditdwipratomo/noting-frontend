...
const colorMap: Record<string, { bg: string; border: string; label: string; value: string; sub: string }> = {
  blue: {
    bg: "bg-blue-50",
...
function StatCard({ stat }: { stat: NutritionStat }) {
  ...
  return (
    <div className={cn("p-3 rounded-2xl border", c.bg, c.border)}>
      ...
      <div className={cn("text-2xl font-[var(--font-display)] font-bold", c.value)}>
